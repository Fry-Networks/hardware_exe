"""Qt widget presenting Presearch Node status."""
...
from miner_GUI.services.presearch import PresearchStatus, PresearchController
from .toggle_switch import ToggleSwitch


class PresearchPanel(QtWidgets.QGroupBox):
    """Panel showing Presearch Node status with toggle."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._last_status: Optional[PresearchStatus] = None
        ...
        self._controller: Optional[PresearchController] = None
        try:
            self._controller = PresearchController()
        except Exception:
            pass
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        # Status label
        self._status_label = QtWidgets.QLabel("Presearch Node: checking status...")
    ...
    def update_status(self, status: PresearchStatus) -> None:
    ...
    def _build_summary(self, status: PresearchStatus) -> str:
        """Build status summary text."""
        from miner_GUI.ui.main_window import PENDING_MESSAGE
        if status.pending:
            return PENDING_MESSAGE
        if not status.configured:
            return "Presearch Node not configured. Get registration code from presearch.io"
    ...
    def _build_stats(self, status: PresearchStatus) -> str:
        """Build statistics text."""
        parts = []

        if status.total_pre_earned > 0:
            parts.append(f"PRE Earned: {status.total_pre_earned:.4f}")

        if status.reliability_score > 0:
            parts.append(f"Reliability: {status.reliability_score:.1f}%")

        if status.total_uptime_hours > 0:
            days = status.total_uptime_hours / 24
            if days >= 1:
                parts.append(f"Uptime: {days:.1f} days")
            else:
                parts.append(f"Uptime: {status.total_uptime_hours:.1f} hrs")

        if status.queries_today > 0:
            parts.append(f"Queries today: {status.queries_today}")
